# AI Biz Solution Team Blog (AI비즈솔루션팀 블로그)

AI비즈솔루션팀의 스터디/업무 적용 사례를 기록하는 정적 블로그입니다.  
디자인은 **네오 브루탈리즘 스타일**을 적용했으며, 포스트는 마크다운 파일로 저장되고 `posts.json` 메타데이터를 통해 인덱싱됩니다.

## 주요 기능

- **네오 브루탈리즘 디자인**: 굵은 테두리, 강한 그림자, 고대비 UI 적용
- **검색 및 필터**: 제목 검색 및 태그 필터링 기능
- **클라이언트 사이드 렌더링**: 로컬 `scripts/vendor/marked.min.js`로 마크다운을 변환하고 `DOMPurify`로 HTML을 정리
- **GitHub Pages 최적화**: 별도의 빌드 과정 없는 순수 정적 구조

## 자료실 활용 (PDS)
`PDS/` 폴더는 **Public Data Storage**의 약자로, 블로그 포스트와 관련된 참고 자료(발표 자료 슬라이드 HTML, PDF 등)를 저장하는 공간입니다. 포스트 내에서 또는 `posts.json`의 `links`를 통해 이 자료들을 링크할 수 있습니다.
...
4.  **공유 미리보기 갱신**: 카카오톡 등 SNS 공유 카드가 글별 제목/요약을 읽을 수 있도록 포스트별 HTML을 다시 생성합니다.
...
포스트 내부에서 **다른 포스트로 이동**하려면 `posts/slug.html` 형식을 사용합니다.

```md
[다른 글로 이동](posts/my-story.html)
```
